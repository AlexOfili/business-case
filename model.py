...
def sensitivity(variable, fixed, trolleys, price=200,
    ...
    All three are derived from ``profit_at`` so the code matches the
    spreadsheet — change any input above and the function follows.
    """
    v_eff = variable * support_mult
    base            = profit_at(price,        v_eff, fixed, trolleys)
    two_x_support   = profit_at(price, 2 *    v_eff, fixed, trolleys)
    churn_floor     = profit_at(price,        v_eff,
                                fixed + churn * trolleys * redeploy_cost,
                                trolleys * (1 - churn))
    return base, two_x_support, churn_floor   # (baseline, 2x-support, churn)
# ---------------------------------------------------------------------------
# Demo / verification
# ---------------------------------------------------------------------------
